...
public class HelloController {
    ...
    //2.帶參數(?xxx=xxx 配置@ResquestParam)
    ...
    */
     ...
     * 子路徑:/calc/sud?x=30&y=20 -> Result: 30-20 = 10
     * 子路徑:/calc/sud?y=20      -> Result:  0-20 =-20
    ...
    public String age(@RequestParam("age") List<Integer> ageList) {
    	double avgOfAge =ageList.stream().mapToLong(Integer::intValue).average().getAsDouble();
    	return String.format("ravg of age=%.1f",avgOfAge);
    }
    /*
     *8.得到多筆Java考試成績的資料(Lab 練習)
     * 路徑:/javaexam?score=80&score=100&score=50
     * 求出最高分、最低分、平均與總分=hight:100, low:50, avg:70.67 ,sum:230
     */
    @GetMapping("/javaexam")
    @ResponseBody
    public String javaexam(@RequestParam("score") List<Integer> scoreList) {   	
    	...
    	return String.format("hight: %d, low: %d, avg: %.2f ,sum: %d",stat.getMax(),stat.getMin(),stat.getAverage(),stat.getSum());
    }
    /*
     *9.pojo(entity)
    ...
    /*
     *10.Map 參數
     * 路徑:/person?name=John&score=100&age=28&pass=true
...
//    }
    ...
    @ResponseBody
    public String getSeesionInfo(HttpSession httpSession) {
